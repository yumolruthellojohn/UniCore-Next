...
export const createReserveRoomColumns = (onDataChange: () => void): ColumnDef<ReserveRoom>[] => [
  {
    accessorKey: 'rq_id',
    header: ({ column }) => (
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} className="w-[20%] min-w-[160px]" title="Room Name" />
    ),
  ...
  },
  {
    ...
    header: 'Actions',
    cell: ({ row }) => {
      const reserveRoom = row.original
      ...
      const handleDelete = async () => {
        try {
          await axios.delete(`http://localhost:8081/requests/${reserveRoom.room_id}`)
          toast({
      ...
      return (
        <div className="flex items-center justify-end space-x-2 w-[20%] min-w-[160px]">
          <Button 
            ...
            size="icon" 
            title="View Details"
            onClick={() => router.push(`/admin/requests/view?id=${reserveRoom.rq_id}`)}
          >
...
export const reserveRoomColumns: ColumnDef<ReserveRoom>[] = createReserveRoomColumns(() => {})