...
export type JobCustodianApproved = {
  job_id: number
  job_rq_id: number
  job_dept_id: number
...
export const createJobCustodianApprovedColumns = (onDataChange: () => void): ColumnDef<JobCustodianApproved>[] => [
  {
    accessorKey: 'job_id',
    header: ({ column }) => (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleView = async () => {
        const url = `/technical/requests/job-requests/view-status?id=${jobQueue.job_id}`; // URL for view-for-approval
        router.push(url);
      }
...
export const jobCustodianApprovalColumns: ColumnDef<JobCustodianApproved>[] = createJobCustodianApprovedColumns(() => {})
