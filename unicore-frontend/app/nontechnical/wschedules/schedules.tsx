...
export default function Schedules({ session }: { session: Session | null }) {
    ...
    const getScheduleData = useCallback(async () => {
        if (userDept) {
            try {
                const sched_response = await axios.get(`http://${ip_address}:8081/schedules/userID/${userID}`)
    ...
    const getEvents = useCallback(async () => {
        try {
            const response = await axios.get(`http://${ip_address}:8081/schedules/userID/${userID}`)
    ...
    const handleEventClick = useCallback((clickInfo: any) => {
        ...
        router.push(`/nontechnical/wschedules/view?id=${scheduleId}`)
    }, [router])
